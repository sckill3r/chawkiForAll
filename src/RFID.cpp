 ...
 * Constructor.
 */
RFID::RFID(): RFID(SS, UINT8_MAX) { // SS is defined in pins_arduino.h, UINT8_MAX means there is no connection from Arduino to RFID's reset and power down input
} // End constructor
 ...
 */
RFID::RFID(	byte resetPowerDownPin	///< Arduino pin connected to RFID's reset and power down input (Pin 6, NRSTPD, active low). If there is no connection from the CPU to NRSTPD, set this to UINT8_MAX. In this case, only soft reset will be used in PCD_Init().
				): RFID(SS, resetPowerDownPin) { // SS is defined in pins_arduino.h
} // End constructor
 ...
 * Prepares the output pins.
 */
RFID::RFID(	byte chipSelectPin,		///< Arduino pin connected to RFID's SPI slave select input (Pin 24, NSS, active low)
					byte resetPowerDownPin	///< Arduino pin connected to RFID's reset and power down input (Pin 6, NRSTPD, active low). If there is no connection from the CPU to NRSTPD, set this to UINT8_MAX. In this case, only soft reset will be used in PCD_Init().
				) {
 ...
 */
void RFID::PCD_WriteRegister(	PCD_Register reg,	///< The register to write to. One of the PCD_Register enums.
									byte value			///< The value to write.
								) {
	SPI.beginTransaction(SPISettings(RFID_SPICLOCK, MSBFIRST, SPI_MODE0));	// Set the settings to work with SPI bus
 ...
 * The interface is described in the datasheet section 8.1.2.
 */
byte RFID::PCD_ReadRegister(	PCD_Register reg	///< The register to read from. One of the PCD_Register enums.
								) {
	byte value;
	SPI.beginTransaction(SPISettings(RFID_SPICLOCK, MSBFIRST, SPI_MODE0));	// Set the settings to work with SPI bus
	digitalWrite(_chipSelectPin, LOW);			// Select slave
 ...
 * The interface is described in the datasheet section 8.1.2.
 */
void RFID::PCD_ReadRegister(	PCD_Register reg,	///< The register to read from. One of the PCD_Register enums.
	...
	SPI.transfer(address);					// Tell RFID which address we want to read
 ...
 */
void RFID::PCD_SetRegisterBitMask(	PCD_Register reg,	///< The register to update. One of the PCD_Register enums.
 ...
 */
void RFID::PCD_ClearRegisterBitMask(	PCD_Register reg,	///< The register to update. One of the PCD_Register enums.
 ...
 */
RFID::StatusCode RFID::PCD_CalculateCRC(	byte *data,		///< In: Pointer to the data to transfer to the FIFO for CRC calculation.
	...
	// 89ms passed and nothing happened. Communication with the RFID might be down.
...
void RFID::PCD_Init() {
	...
	if (_resetPowerDownPin != UNUSED_PIN) {
		// First set the resetPowerDownPin as digital input, to check the RFID power down mode.
		pinMode(_resetPowerDownPin, INPUT);
	
		if (digitalRead(_resetPowerDownPin) == LOW) {	// The RFID chip is in power down mode.
 ...
 * Initializes the RFID chip.
 */
void RFID::PCD_Init(	byte resetPowerDownPin	///< Arduino pin connected to RFID's reset and power down input (Pin 6, NRSTPD, active low)
					) {
 ...
 * Initializes the RFID chip.
 */
void RFID::PCD_Init(	byte chipSelectPin,		///< Arduino pin connected to RFID's SPI slave select input (Pin 24, NSS, active low)
						byte resetPowerDownPin	///< Arduino pin connected to RFID's reset and power down input (Pin 6, NRSTPD, active low)
					) {
	_chipSelectPin = chipSelectPin;
	_resetPowerDownPin = resetPowerDownPin; 
	// Set the chipSelectPin as digital output, do not select the slave yet
...
/**
 * Performs a self-test of the RFID
 * See 16.1.1 in http://www.nxp.com/documents/data_sheet/RFID.pdf
 * 
...
bool RFID::PCD_PerformSelfTest() {
	...
	switch (version) {
		...
		case 0x90:	// Version 0.0
			reference = RFID_firmware_referenceV0_0;
			break;
		case 0x91:	// Version 1.0
			reference = RFID_firmware_referenceV1_0;
			break;
		case 0x92:	// Version 2.0
			reference = RFID_firmware_referenceV2_0;
			break;
...
void RFID::PCD_SoftPowerDown(){//Note : Only soft power down mode is available throught software
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
RFID::StatusCode RFID::PCD_TransceiveData(	byte *sendData,		///< Pointer to the data to transfer to the FIFO.
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
RFID::StatusCode RFID::PCD_CommunicateWithPICC(	byte command,		///< The command to execute. One of the PCD_Command enums.
	...
	// 36ms and nothing happened. Communication with the RFID might be down.
	if (!completed) {
		return STATUS_TIMEOUT;
	}
	...
	if (backData && backLen && checkCRC) {
		...
		RFID::StatusCode status = PCD_CalculateCRC(&backData[0], *backLen - 2, &controlBuffer[0]);
		if (status != STATUS_OK) {
			return status;
		}
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
RFID::StatusCode RFID::PICC_RequestA(	byte *bufferATQA,	///< The buffer to store the ATQA (Answer to request) in
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
RFID::StatusCode RFID::PICC_WakeupA(	byte *bufferATQA,	///< The buffer to store the ATQA (Answer to request) in
											byte *bufferSize	///< Buffer size, at least two bytes. Also number of bytes returned if STATUS_OK.
										) {
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */ 
RFID::StatusCode RFID::PICC_REQA_or_WUPA(	byte command, 		///< The command to send - PICC_CMD_REQA or PICC_CMD_WUPA
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
RFID::StatusCode RFID::PICC_Select(	Uid *uid,			///< Pointer to Uid struct. Normally output, but can also be used to supply a known UID.
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */ 
RFID::StatusCode RFID::PICC_HaltA() {
	RFID::StatusCode result;
	byte buffer[4];
 ...
 */
RFID::StatusCode RFID::PCD_Authenticate(byte command,		///< PICC_CMD_MF_AUTH_KEY_A or PICC_CMD_MF_AUTH_KEY_B
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
RFID::StatusCode RFID::MIFARE_Read(	byte blockAddr, 	///< MIFARE Classic: The block (0-0xff) number. MIFARE Ultralight: The first page to return data from.
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
RFID::StatusCode RFID::MIFARE_Write(	byte blockAddr, ///< MIFARE Classic: The block (0-0xff) number. MIFARE Ultralight: The page (2-15) to write to.
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
RFID::StatusCode RFID::MIFARE_Ultralight_Write(	byte page, 		///< The page (2-15) to write to.
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
RFID::StatusCode RFID::MIFARE_Decrement(	byte blockAddr, ///< The block (0-0xff) number.
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
RFID::StatusCode RFID::MIFARE_Increment(	byte blockAddr, ///< The block (0-0xff) number.
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
RFID::StatusCode RFID::MIFARE_Restore(	byte blockAddr ///< The block (0-0xff) number.
											) {
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
RFID::StatusCode RFID::MIFARE_TwoStepHelper(	byte command,	///< The command to use
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
RFID::StatusCode RFID::MIFARE_Transfer(	byte blockAddr ///< The block (0-0xff) number.
											) {
	RFID::StatusCode result;
	byte cmdBuffer[2]; // We only need room for 2 bytes.
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
  */
RFID::StatusCode RFID::MIFARE_GetValue(byte blockAddr, int32_t *value) {
	RFID::StatusCode status;
	byte buffer[18];
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
RFID::StatusCode RFID::MIFARE_SetValue(byte blockAddr, int32_t value) {
	byte buffer[18];
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
RFID::StatusCode RFID::PCD_NTAG216_AUTH(byte* passWord, byte pACK[]) //Authenticate with 32bit password
{
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
RFID::StatusCode RFID::PCD_MIFARE_Transceive(	byte *sendData,		///< Pointer to the data to transfer to the FIFO. Do NOT include the CRC_A.
 ...
 */
const __FlashStringHelper *RFID::GetStatusCodeName(RFID::StatusCode code	///< One of the StatusCode enums.
										) {
	switch (code) {
 ...
 * @return PICC_Type
 */
RFID::PICC_Type RFID::PICC_GetType(byte sak		///< The SAK byte returned from PICC_Select().
										) {
 ...
 * @return const __FlashStringHelper *
 */
const __FlashStringHelper *RFID::PICC_GetTypeName(PICC_Type piccType	///< One of the PICC_Type enums.
													) {
	switch (piccType) {
 ...
 */
void RFID::PCD_DumpVersionToSerial() {
	// Get the RFID firmware version
	...
	if ((v == 0x00) || (v == 0xFF))
		Serial.println(F("WARNING: Communication failure, is the RFID properly connected?"));
 ...
 */
void RFID::PICC_DumpToSerial(Uid *uid	///< Pointer to Uid struct returned from a successful PICC_Select().
								) {
	MIFARE_Key key;
 ...
 */
void RFID::PICC_DumpDetailsToSerial(Uid *uid	///< Pointer to Uid struct returned from a successful PICC_Select().
									) {
	// UID
 ...
 * On success the PICC is halted after dumping the data.
 */
void RFID::PICC_DumpMifareClassicToSerial(	Uid *uid,			///< Pointer to Uid struct returned from a successful PICC_Select().
 ...
 */
void RFID::PICC_DumpMifareClassicSectorToSerial(Uid *uid,			///< Pointer to Uid struct returned from a successful PICC_Select().
 ...
 * not recognise anymore (see RFID::MIFARE_UnbrickUidSector).
 * 
...
bool RFID::MIFARE_OpenUidBackdoor(bool logErrors) {
	...
	RFID::StatusCode status = PCD_TransceiveData(&cmd, (byte)1, response, &received, &validBits, (byte)0, false); // 40
	if(status != STATUS_OK) {
		if(logErrors) {
...
bool RFID::MIFARE_SetUid(byte *newUid, byte uidSize, bool logErrors) {
	...
	RFID::StatusCode status = PCD_Authenticate(RFID::PICC_CMD_MF_AUTH_KEY_A, (byte)1, &key, &uid);
	if (status != STATUS_OK) {
...
bool RFID::MIFARE_UnbrickUidSector(bool logErrors) {
	...
	// Write modified block 0 back to card
	RFID::StatusCode status = MIFARE_Write((byte)0, block0_buffer, (byte)16);
	if (status != STATUS_OK) {
		if (logErrors) {
			Serial.print(F("MIFARE_Write() failed: "));
			Serial.println(GetStatusCodeName(status));
...
bool RFID::PICC_IsNewCardPresent() {
	...
	RFID::StatusCode result = PICC_RequestA(bufferATQA, &bufferSize);
...
bool RFID::PICC_ReadCardSerial() {
	RFID::StatusCode result = PICC_Select(&uid);
